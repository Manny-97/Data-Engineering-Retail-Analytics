...
from (
    select 
        Number_of_customers,
        customer_country,
        PERCENTILE_CONT(0.9) WITHIN GROUP (ORDER BY Number_of_customers DESC) OVER () AS Top_Ten_Percentile
    from CustomerCounts
) as Subquery
where Number_of_customers >= Top_Ten_Percentile
order by customer_country ASC;

'''

top_ten_percentile = sql(query)
